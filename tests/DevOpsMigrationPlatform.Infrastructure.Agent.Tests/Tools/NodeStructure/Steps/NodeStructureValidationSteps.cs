...
[Binding]
[Scope(Feature = "NodeStructure configuration and package validation")]
public class NodeStructureValidationSteps
{
    private readonly NodeStructureValidationContext _ctx;
    private string? _invalidRegexPattern;

    public NodeStructureValidationSteps(NodeStructureValidationContext ctx) => _ctx = ctx;

    [Given(@"a package with referenced-paths\.json and NodeStructure configuration")]
    ...
    [Given(@"a NodeStructure mapping rule maps ""(.*)"" to ""(.*)""")]
    ...
    [Given(@"a NodeStructure mapping rule with an invalid regex pattern ""(.*)""")]
