...
[TestClass]
public class NodeStructureModuleTests
{
    private static NodeStructureModule CreateModule(
        NodeStructureModuleOptions? options = null,
        IClassificationTreeCapture? capture = null,
        INodeEnsurer? nodeEnsurer = null)
    {
        options ??= new NodeStructureModuleOptions { Enabled = true };
        return new NodeStructureModule(
            NullLogger<NodeStructureModule>.Instance,
    ...
    public async Task ExportAsync_Skips_WhenModuleDisabled()
    {
        // Arrange
        var captureMock = new Mock<IClassificationTreeCapture>(MockBehavior.Strict);
        var module = CreateModule(new NodeStructureModuleOptions { Enabled = false }, captureMock.Object);
        var store = Mock.Of<IArtefactStore>();
        var context = CreateExportContext(store);
    ...
    public async Task ImportAsync_CallsReplicateSourceTree_WhenOptionEnabled()
    {
        ...
        var opts = new NodeStructureModuleOptions { Enabled = true, ReplicateSourceTree = true };
    ...
    public async Task ImportAsync_CallsEnsureReferencedPaths_WhenAutoCreateNodesEnabled()
    {
        ...
        var opts = new NodeStructureModuleOptions { Enabled = true, AutoCreateNodes = true };
        var module = CreateModule(opts, nodeEnsurer: ensurerMock.Object);
        var store = Mock.Of<IArtefactStore>();
        var context = CreateImportContext(store);
    ...
    public async Task ImportAsync_CallsNeitherMethod_WhenBothOptionsDisabled()
    {
        ...
        var opts = new NodeStructureModuleOptions
        {
            Enabled = true,
