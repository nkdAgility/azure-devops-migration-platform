...
{
    private const string RevisionFolderPath = "2026-04-11/123-12345-r0";
    ...
    public void GivenTheTestProjectIsReadyForExport()
    {
        ...
        _context.Package = CreatePackageAccess(_context.PackageRoot, "embedded-images-tests");
    ...
    public async Task WhenTheExportRuns()
    {
        ...
        _context.MockDownloader
            ...
            {
                if (url.StartsWith("https://example.com", StringComparison.OrdinalIgnoreCase) && !url.Contains("deleted404", StringComparison.OrdinalIgnoreCase))
                {
                ...
                if (url.Contains("deleted404", StringComparison.OrdinalIgnoreCase))
                {
        ...
        var exportService = new EmbeddedImageExportService(
            ...
            _context.Package,
            CreateSourceEndpointInfo().Object,
            mockLogger.Object);
    ...
    {
        Assert.IsTrue(Directory.Exists(RevisionDirectory), $"Revision folder should exist at {RevisionDirectory}");
        var imageFiles = Directory.GetFiles(RevisionDirectory, "image-*.png");
    ...
    {
        var hasWarning = _context.LoggedWarnings.Any(w => w.Contains("external-image.png", StringComparison.OrdinalIgnoreCase) ||
            w.Contains("Could not download image", StringComparison.OrdinalIgnoreCase));
    ...
    {
        var has404Warning = _context.LoggedWarnings.Any(w => w.Contains("404", StringComparison.OrdinalIgnoreCase));
    ...
    private static Mock<ISourceEndpointInfo> CreateSourceEndpointInfo()
    {
        var endpoint = new Mock<ISourceEndpointInfo>(MockBehavior.Strict);
        endpoint.SetupGet(x => x.Url).Returns(string.Empty);
        endpoint.SetupGet(x => x.Project).Returns(string.Empty);
        endpoint.SetupGet(x => x.ConnectorType).Returns("AzureDevOpsServices");
        return endpoint;
    }
