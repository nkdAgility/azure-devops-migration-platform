...
19. **TFS Object Model runs in an isolated subprocess only.**
    The .NET 4.x TFS exporter (`DevOpsMigrationPlatform.TfsExporter`) is a completely separate binary and a full export executor for TFS sources. It connects to TFS via the TFS OM, writes `revision.json` and attachments directly to the package path (via `IArtefactStore` / `IMigrationRepository` compiled for net481), and maintains its own cursor (SQLite watermark store). It reports progress to the .NET 10 host via NDJSON on stdout; errors go to stderr. The .NET 10 host must never hold a compiled reference to the .NET 4 project, link against any .NET Framework assembly, or invoke TFS OM APIs in any form. The adapter (`TfsExporterProcessAdapter`) is the only permitted caller of the subprocess. The subprocess receives the package output path and credentials via CLI args and stdin; it writes package artefacts directly to that path. See [docs/tfs-exporter.md](../../docs/tfs-exporter.md) for the full protocol specification.
