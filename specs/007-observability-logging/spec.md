...
| `docs/tui.md` | Discrepancy — references `GET /jobs/{jobId}/logs` for progress events; no diagnostics panel documented |
| `docs/control-plane.md` | Discrepancy — progress endpoint named `/logs`; no diagnostics endpoint; no download endpoint |
| `docs/migration-agent.md` | Confirmed accurate — already documents three sinks and `Logs/` package writing |
| `docs/cli.md` | Discrepancy — `manage logs` command conflates progress events with diagnostic logs |
| `.agents/guardrails/system-architecture.md` | Confirmed accurate — guardrail #12 (agents stateless, durable state in package), #13 (IArtefactStore only), #18 (no UI coupling) all apply |
| `.agents/context/artefact-store.md` | Confirmed accurate — IArtefactStore is the only permitted file abstraction |
| `.agents/context/package-format.md` | Discrepancy — `Logs/` folder listed but contents not specified |
...
- Architecture docs read: `docs/architecture.md`, `docs/tui.md`, `docs/control-plane.md`, `docs/migration-agent.md`, `docs/cli.md`, `docs/aspire-integration.md`, `.agents/guardrails/system-architecture.md`, `.agents/context/artefact-store.md`, `.agents/context/package-format.md`.
