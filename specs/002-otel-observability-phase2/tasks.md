...
- [ ] T010 [P] [US2] Create `features/platform/telemetry/progress-controller.feature` — translate US2 acceptance scenario 2 and FR-014 into Gherkin: event retrievable immediately via `GET /jobs/{jobId}/logs`; 403 returned when caller lacks job visibility
...
- [ ] T018 [US2] Modify `src/DevOpsMigrationPlatform.MigrationAgent/Program.cs` — inside the `controlPlaneBaseUrl` block (after `AddControlPlaneTelemetryClient`), call `builder.Services.AddControlPlaneProgressSink(controlPlaneBaseUrl)`; then register the composite: `builder.Services.AddSingleton<IProgressSink>(sp => new CompositeProgressSink(sp.GetRequiredService<ILogger<CompositeProgressSink>>(), new AnsiProgressSink(), new PackageProgressSink(), sp.GetRequiredService<ControlPlaneProgressSink>()))` so all three sinks receive every `ProgressEvent` — depends on T015, T017
