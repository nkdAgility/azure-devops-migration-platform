...
- **FR-I05**: `IIdentityMappingService.Resolve(sourceIdentity)` MUST apply resolution in this order: (1) explicit `mapping.json` entry, (2) automatic match by UPN in target directory, (3) automatic match by display name in target directory, (4) configured default identity. Unresolved identities MUST be recorded in `Identities/unresolved.json`. If automatic display name matching finds **multiple** target identities with the same display name, all are treated as unresolved (the operator must provide an explicit mapping in `mapping.json`).
...
- **FR-N10**: The module MUST normalise localised root names (e.g., German `Bereich` → English `Area`) during export. The normalisation map is maintained in `NodeStructureModuleOptions.LocaleRootNameMap` and covers German, French, Spanish, Portuguese, and Italian root names by default. Operators can extend it via configuration.
...
- **FR-011**: System MUST support two scope types: `"teams"` (with optional `filter` parameter for case-insensitive substring matching on team display name) and `"all"` (all teams in the project). A team matches when its display name contains the filter string.
