...
- [X] T036 Update `.agents/context/workitems-format.md` to document `<ticks>-<workItemId>-c<commentId>/comment.json` sub-folder, comment version folder layout, and embedded image files beside `revision.json`
- [X] T037 [P] Update `.agents/context/package-format.md` to document embedded-image files beside documents and `Checkpoints/workitems-comments.cursor.json` cursor
