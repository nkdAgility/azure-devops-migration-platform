...
- [X] T005 [P] Create `src/DevOpsMigrationPlatform.Abstractions/Options/OrganisationEntry.cs` — `Type`, `OrgOrCollection`, `Projects` (List\<string\>), `ApiVersion`, `Authentication` (EndpointAuthenticationOptions), `Enabled` (bool, default true) per data-model.md
- [X] T006 [P] Create `src/DevOpsMigrationPlatform.Abstractions/Models/InventorySummary.cs` — all 9 CSV fields: `OrgOrCollection`, `ProjectName`, `WorkItemsCount`, `RevisionsCount`, `ReposCount`, `PipelinesCount`, `IsComplete`, `Error`, `LastUpdatedUtc` per data-model.md
- [X] T007 [P] Create `src/DevOpsMigrationPlatform.Abstractions/Models/InventoryProgressEvent.cs` — `ProjectName`, `OrgOrCollection`, `WorkItemsCount`, `RevisionsCount`, `IsComplete`, `WindowStart`, `WindowEnd`, `WindowSize`, `Error`, `Timestamp` per data-model.md
...
- [X] T011 Create `src/DevOpsMigrationPlatform.Abstractions/Services/IInventoryService.cs` — `IAsyncEnumerable<InventoryProgressEvent> CountWorkItemsAsync(string orgOrCollection, string project, string pat, CancellationToken ct)` per data-model.md
...
- [X] T016 [US1] Rewrite `src/DevOpsMigrationPlatform.CLI.Migration/Commands/Discovery/InventoryCommand.cs` — `Settings` with `[CommandOption] --config`, `[CommandOption] --output` (default CWD), `[CommandOption] --all-projects`; load `IOptions<InventoryOptions>`; validate on execute; Mode 1 path uses `IInventoryService`; render `LiveTable` via Spectre.Console with columns: OrgOrCollection, Project, Work Items, Revisions, Repos, Pipelines, Updated
...
- [X] T018 [US1] Implement CSV write on command completion in `src/DevOpsMigrationPlatform.CLI.Migration/Commands/Discovery/InventoryCommand.cs` — write `discovery-summary.csv` to `--output` directory; all 9 columns (`OrgOrCollection,ProjectName,WorkItemsCount,RevisionsCount,ReposCount,PipelinesCount,IsComplete,Error,LastUpdatedUtc`); all projects written including failed ones
