...
- Q: Do Inventory and Discovery already load the full work item (making filter evaluation free)? → A: No. Verified in code: `AzureDevOpsWorkItemDiscoveryService.DiscoverWorkItemsAsync` only fetches `System.Rev` via `GetWorkItemsAsync`. Filter-referenced field names must be unioned into the `fields:` array of that batch call. The `IWorkItemDiscoveryService.DiscoverWorkItemsAsync` interface will accept a new optional `IReadOnlyList<WorkItemFieldFilterOptions>? filterOptions = null` parameter (breaking change — acceptable, platform-controlled). TFS implementation ignores filter options.
...
- **FR-002**: The export orchestrator MUST evaluate all `filter` scopes against the current work item state **before** requesting revision history; if a work item fails the filter, no revision history is fetched. All ADO fields from the current work item object are available for filter evaluation.
...
- **FR-006**: `OrganisationEntry` MUST accept a `scopes` array of the same scope type structure used in module configuration
- **FR-007**: An organisation `wiql` scope MUST supply its `query` parameter to the work item discovery service for that organisation, replacing the platform default query
...
- **FR-009**: The `filter` scope on organisations MUST be evaluated during inventory and discovery operations using the full work item data already loaded by those operations; no additional API calls are required for filter evaluation
...
- **FilterScope**: A configuration entry with `mode` (`include`/`exclude`), `field` (string field reference), and `pattern` (regex string). Belongs to a `WorkItems` module's `scopes` array or an `OrganisationEntry`'s `scopes` array.
- **WiqlScope**: An existing scope type with a `query` parameter. Extended to be usable on `OrganisationEntry` in addition to its current home on module scopes.
- **OrganisationEntry.Scopes**: New property — ordered list of scopes (initially `wiql` and `filter` types) that refine which work items an organisation operates on during discovery/inventory.
...
- **SC-006**: An inventory run with filter scopes configured produces a batch request to `GetWorkItemsAsync` whose `fields` array contains exactly `System.Rev` unioned with the filter-referenced field names — verifiable by unit test. No additional API calls are made beyond the existing batch. Documentation MUST warn users to prefer short fields and minimise filter count.
...
- The regex engine uses a fixed internal 2-second timeout per evaluation to guard against catastrophic backtracking; this is not user-configurable. `RegexMatchTimeoutException` is caught by `WorkItemFieldFilterChain`, logged as a warning, and treated as a non-match (include = fail, exclude = pass); migration continues uninterrupted
- Field values are compared as their string representation; no type coercion is performed before regex matching
- During export, filter is evaluated on the current work item state before revision history is fetched; filtered-out items incur no revision API calls
- During import, only the last revision's fields are evaluated by filter scopes; a lightweight pre-pass enumerates revision folder names only (no file reads) to identify the last folder per work item, then reads only those `revision.json` files before the main streaming pass
- Inventory fetches work items in batches via `GetWorkItemsAsync` with `fields: ["System.Rev"]`. When filter scopes are present, the filter-referenced field names are unioned into that `fields:` array — no extra API calls, just a slightly larger batch response. Discovery uses a separate link-analysis path; the same pattern applies if a batch field fetch exists there (verified in T020a)
