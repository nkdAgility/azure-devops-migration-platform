...
`DiscoveryJobAuthentication.Type` is a `string` with default `"AccessToken"`. The config layer uses `AuthenticationType` (an enum: `None`, `AccessToken`, `Windows`). The new `OrganisationEndpointAuthentication` uses `AuthenticationType Type` (the enum).
...
The CLI command mapping (currently `Type = "AccessToken"`) will change to `Type = AuthenticationType.AccessToken`.
