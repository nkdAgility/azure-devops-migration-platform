...
- [X] T001 Add `Microsoft.Data.Sqlite` package reference to `Directory.Packages.props` and to `src/DevOpsMigrationPlatform.Infrastructure/DevOpsMigrationPlatform.Infrastructure.csproj` — Status: complete
- [X] T002 [P] Create import scenario config file at `scenarios/import-ado-workitems-single-project.json` with `mode: Import`, `target.type: Simulated`, and WorkItems module with all extensions enabled — Status: complete
- [X] T003 [P] Add `.vscode/launch.json` debug profile for import scenario (name: `📥 Migration CLI: Queue Import (Simulated)`, args: `queue --config scenarios/import-ado-workitems-single-project.json`) — Status: complete
...
- [X] T004 Create `IdMapEntry` record in `src/DevOpsMigrationPlatform.Abstractions/Models/IdMapEntry.cs` with `SourceId` (int) and `TargetId` (int) properties — Status: complete
- [X] T005 [P] Create `AttachmentMapEntry` record in `src/DevOpsMigrationPlatform.Abstractions/Models/AttachmentMapEntry.cs` with `SourceWorkItemId`, `RevisionIndex`, `RelativePath`, `TargetAttachmentId` properties — Status: complete
- [X] T006 [P] Create `ImportedWorkItemResult` record in `src/DevOpsMigrationPlatform.Abstractions/Models/ImportedWorkItemResult.cs` with `TargetWorkItemId` (int) and `IsNewlyCreated` (bool) properties — Status: complete
- [X] T007 [P] Create `EmbeddedImageMetadata` record in `src/DevOpsMigrationPlatform.Abstractions/Models/EmbeddedImageMetadata.cs` with `OriginalUrl`, `RelativePath`, `Extension`, `Sha256`, `Size` properties — Status: complete
- [X] T008 [P] Create `WorkItemRelations` record in `src/DevOpsMigrationPlatform.Abstractions/Models/WorkItemRelations.cs` with `RelatedLinks`, `ExternalLinks`, `Hyperlinks` read-only list properties — Status: complete
- [X] T009 [P] Add `EmbeddedImages` property (`IReadOnlyList<EmbeddedImageMetadata>`) to `WorkItemRevision` record in `src/DevOpsMigrationPlatform.Abstractions/Models/WorkItemRevision.cs` — Status: complete
- [X] T010 Create `IIdMapStore` interface in `src/DevOpsMigrationPlatform.Abstractions/Services/IIdMapStore.cs` with `InitializeAsync`, `GetTargetWorkItemIdAsync`, `SetWorkItemMappingAsync`, `GetAttachmentIdAsync`, `SetAttachmentMappingAsync`, `SeedWorkItemMappingsAsync` methods per contracts/IWorkItemImportTarget.md — Status: complete
...
- [X] T012 [P] Create `IWorkItemImportTargetFactory` interface in `src/DevOpsMigrationPlatform.Abstractions/Services/IWorkItemImportTargetFactory.cs` with `CreateAsync(orgUrl, project, accessToken, ct)` method — Status: complete
- [X] T013 [P] Create `IWorkItemResolutionStrategy` interface in `src/DevOpsMigrationPlatform.Abstractions/Services/IWorkItemResolutionStrategy.cs` with `SeedAsync`, `ResolveSingleAsync`, `WriteProvenanceAsync` methods per contracts/IWorkItemImportTarget.md — Status: complete
- [X] T014 Implement `SqliteIdMapStore` in `src/DevOpsMigrationPlatform.Infrastructure/Import/SqliteIdMapStore.cs` — creates/opens `Checkpoints/idmap.db` via `IArtefactStore`, implements `IIdMapStore` with `work_item_map` and `attachment_map` tables per data-model.md SQLite schema — Status: complete
- [X] T015 [P] Implement `NullResolutionStrategy` in `src/DevOpsMigrationPlatform.Infrastructure.AzureDevOps/Import/NullResolutionStrategy.cs` — `SeedAsync` is no-op, `ResolveSingleAsync` returns null, `WriteProvenanceAsync` is no-op (FR-023 default fallback) — Status: complete
- [X] T015b [P] Implement `SimulatedWorkItemImportTarget` in `src/DevOpsMigrationPlatform.Infrastructure/Import/SimulatedWorkItemImportTarget.cs` — minimal in-memory `IWorkItemImportTarget` for offline testing. `CreateWorkItemAsync` assigns auto-incrementing target IDs, validates fields are non-empty, returns `ImportedWorkItemResult`. `UpdateFieldsAsync`, `AddLinksAsync`, `CreateCommentAsync` validate inputs and log calls. `UploadAttachmentAsync` / `UploadEmbeddedImageAsync` return deterministic fake URLs. `GetExistingRelationsAsync` returns empty `WorkItemRelations`. Registers via `AddSimulatedImportServices()` when `target.type == "Simulated"`. Required by T002/T050 scenario config. — Status: complete
...
- [X] T025 [US2] Create `features/platform/checkpointing/import-cursor-resume.feature` — translate spec.md User Story 2 acceptance scenarios 1–3 into conformant Gherkin per `.agents/20-guardrails/workflow/acceptance-test-format.md` (Feature: Import Cursor Resume) — Status: complete

### Implementation for User Story 2

- [X] T026 [US2] Add mid-folder resume logic to `RevisionFolderProcessor.ProcessAsync` in `src/DevOpsMigrationPlatform.Infrastructure/Import/RevisionFolderProcessor.cs` — when the cursor `lastProcessed` matches the current folder and `stage` is not `Completed`, skip stages already completed (e.g. if stage is `AppliedFields`, start from `AppliedLinks`). This extends the stage loop added in T017. — Status: complete
- [X] T027 [US2] Add `--force-fresh` handling for import cursor in `WorkItemImportOrchestrator` in `src/DevOpsMigrationPlatform.Infrastructure/Import/WorkItemImportOrchestrator.cs` — when `MigrationJob.Resume.Mode` is `ForceFresh`, delete `Checkpoints/workitems.cursor.json` via `IStateStore` before starting enumeration. Preserve `idmap.db` (do not delete). — Status: complete
...
- [X] T028 [US3] Create `features/import/work-items/revisions/streaming-memory-safety.feature` — translate spec.md User Story 3 acceptance scenarios 1–3 into conformant Gherkin per `.agents/20-guardrails/workflow/acceptance-test-format.md` (Feature: Streaming Memory-Safe Import) — Status: complete
...
- [X] T030 [US4] Create `features/services/identity-mapping/import-identity-resolution.feature` — translate spec.md User Story 4 acceptance scenarios 1–3 into conformant Gherkin per `.agents/20-guardrails/workflow/acceptance-test-format.md` (Feature: Identity Resolution During Import) — Status: complete
...
- [X] T042 Update `docs/work-item-iteration-guide.md` — add "Import Pattern: WorkItemImportOrchestrator" section with `IWorkItemImportTarget`, `IIdMapStore`, `RevisionFolderProcessor`, and streaming import flow (mirrors existing "Export Pattern" section). Resolves discrepancy #3 and #7. — Status: complete
- [ ] T043 [P] Update `docs/configuration-reference.md` — add `WorkItemResolutionStrategy` extension type to the WorkItems Module Scopes and Extensions table with `strategy` (`TargetField` | `TargetHyperlink`), `fieldName`, and `urlPattern` parameters. Resolves discrepancy #6. — Status: incomplete
  - Evidence: WorkItems extensions table in docs/configuration-reference.md still lacks WorkItemResolutionStrategy rows (docs/configuration-reference.md:275-279).
- [X] T044 [P] Update `.agents/30-context/domains/cli-commands.md` — confirm `queue` command documentation reflects that `Import` and `Both` modes are now functional (no longer stubbed). No new commands added. — Status: complete
- [X] T045 [P] Update `docs/cli-guide.md` — confirm import mode is documented as functional. No new commands. — Status: complete
- [ ] T046 Mark all items in `specs/013-ado-workitems-import/discrepancies.md` as `Resolved` or `N/A` — Status: incomplete
  - Evidence: discrepancies.md items are not individually marked Resolved/N/A; file has no per-item status markers.
- [X] T047 Review `analysis/pending-actions.md` and remove any items resolved by this spec — Status: complete
- [X] T048 Run `dotnet clean && dotnet build --no-incremental` — MUST pass — Status: complete
- [ ] T049 Run `dotnet test` — ALL tests MUST pass — Status: incomplete
  - Evidence: A full `dotnet test DevOpsMigrationPlatform.slnx --no-build` run did not complete in this reconciliation session (stalled during ControlPlane tests).
- [ ] T050 Run scenario config `scenarios/import-ado-workitems-single-project.json` via the `.vscode/launch.json` import debug profile and verify observable output (work items created in simulated target, cursor written, idmap populated, progress events emitted) — Status: incomplete
  - Evidence: No fresh scenario execution evidence was produced in this reconciliation session for scenarios/import-ado-workitems-single-project.json.
- [ ] T051 Add `ValidateAsync` implementation to `WorkItemsModule` in `src/DevOpsMigrationPlatform.Infrastructure/Modules/WorkItemsModule.cs` — Tier 2 pre-flight: verify `WorkItems/` folder exists, at least one revision folder, `manifest.json` compatible. Tier 3 post-flight: target work item count matches unique source IDs, sampled link/attachment verification. Required by SC-004. — Status: incomplete
  - Evidence: ValidateAsync currently performs only Tier 2 WorkItems folder presence checks; Tier 3 count/link/attachment verification is not implemented (WorkItemsModule.cs:661-681).
...
- [X] T052 [P] Add logging and metrics instrumentation to import orchestrator and processor — structured log entries for each stage transition, OpenTelemetry counters for work items created/updated/skipped, histogram for per-folder processing time. — Status: complete
