...
- [ ] T010 [P] Create `features/platform/telemetry/metric-snapshot-relay.feature` — Gherkin scenarios for: (a) Migration Agent pushes `MetricSnapshot` to `POST /agents/lease/{id}/telemetry` on interval, (b) Control Plane stores latest snapshot per job, (c) old snapshot replaced when new one arrives
- [ ] T011 [P] Create `features/platform/telemetry/tui-metrics-panel.feature` — Gherkin scenarios for: (a) `GET /jobs/{jobId}/telemetry` returns `204` when no snapshot received, (b) returns `200` with snapshot after agent pushes one, (c) TUI panel displays metric values from snapshot
