...
- **Suggested update**: Update `.agents/30-context/domains/identity-and-mapping.md` and `.agents/30-context/domains/checkpointing-summary.md` to reflect the actual SQLite-backed implementation: `idmap.db (SQLite — package-local indexed storage, not a control-plane database)`.
...
- **Suggested update**: Defer to planning phase — determine whether these should be explicit CLI commands or remain implicit import-startup behaviour. If CLI commands are added, document them in `docs/cli-guide.md` and `.agents/30-context/domains/cli-commands.md`.
