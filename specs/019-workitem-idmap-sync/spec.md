...
- **FR-017**: At import job startup, the system MUST acquire an exclusive package-level lock (`Checkpoints/agent.lock` containing the job ID and agent instance ID — a GUID generated by `JobAgentWorker` on startup). If a lock file already exists and the owning agent instance is confirmed live (via ControlPlane `GET /agents/{agentInstanceId}/status`), the system MUST hard-bounce the incoming job — fail-fast with a structured error logged via OpenTelemetry and refuse to proceed. The running job is left undisturbed. If the lock file exists but the owning agent instance is no longer active (stale lock), the system MUST replace it and proceed.
...
- **SC-002**: *(Removed — untestable in CI; streaming requirement is captured by FR-013.)*
...
- **SC-005**: *(Removed — memory boundedness is an architectural requirement enforced by FR-013 and the guardrails, not a separately measurable CI criterion.)*
