...
- [ ] T001 Extend `src/DevOpsMigrationPlatform.Abstractions/Models/IdMapEntry.cs` — add `public int? LastRevisionIndex { get; init; }` property (nullable; null = not yet tracked). See `data-model.md` → "Record: IdMapEntry".
...
- [ ] T003 Extend `src/DevOpsMigrationPlatform.Abstractions/Services/IWorkItemImportTarget.cs` — add `Task<bool> WorkItemExistsAsync(int targetWorkItemId, CancellationToken ct)` with XML doc-comment. See `contracts/IWorkItemImportTarget-additions.md`.

- [ ] T004 Create `src/DevOpsMigrationPlatform.Abstractions/Services/IPackageLockService.cs` — new interface with single method `Task<IAsyncDisposable> AcquireAsync(string packagePath, string jobId, CancellationToken ct)` and XML doc-comment. See `contracts/IPackageLockService.md` and `data-model.md` → "IPackageLockService".

- [ ] T005 Create `src/DevOpsMigrationPlatform.Abstractions/Errors/PackageLockConflictException.cs` — sealed exception with properties `PackagePath`, `OwnerJobId`, `OwnerPid`, `AcquiredAt`. Must inherit from `Exception` (not `MigrationException` — this is a startup abort, not a migration error). See `data-model.md` → "PackageLockConflictException".
...
- [ ] T007 Update `InitializeAsync` in `src/DevOpsMigrationPlatform.Infrastructure/Import/SqliteIdMapStore.cs`:
...
- [ ] T008 Implement the 4 new `IIdMapStore` methods in `src/DevOpsMigrationPlatform.Infrastructure/Import/SqliteIdMapStore.cs`:
...
- [ ] T009 [P] Implement `WorkItemExistsAsync` in `src/DevOpsMigrationPlatform.Infrastructure.AzureDevOps/Import/AzureDevOpsWorkItemImportTarget.cs` — call the ADO REST API `GET _apis/wit/workitems/{id}` via the existing work item tracking client; return `false` for a 404 (work item not found), `true` for a 200. Do NOT call `ResolveSingleAsync`. See `contracts/IWorkItemImportTarget-additions.md`.

- [ ] T010 [P] Implement `WorkItemExistsAsync` in `src/DevOpsMigrationPlatform.Infrastructure.Simulated/Import/SimulatedWorkItemImportTarget.cs` — always return `Task.FromResult(true)` (the simulated target never deletes work items). See `contracts/IWorkItemImportTarget-additions.md`.

### Infrastructure — PackageLockFileService

- [ ] T011 Create `src/DevOpsMigrationPlatform.Infrastructure/Services/PackageLockFileService.cs` — implements `IPackageLockService`. Acquire algorithm:
  1. Build lock path = `{packagePath}/Checkpoints/agent.lock`
  2. If file exists: deserialize JSON to read `pid` and `jobId`. Check `Process.GetProcessById(pid)` — if process alive → throw `PackageLockConflictException`. If process dead → delete file.
  3. Write lock file atomically using `FileStream(path, FileMode.CreateNew)` with `JsonSerializer`. Lock file JSON: `{ "jobId": jobId, "pid": Environment.ProcessId, "acquiredAt": DateTimeOffset.UtcNow }`.
  4. Return an `IAsyncDisposable` handle that deletes the lock file on `DisposeAsync`.
  Direct `FileStream` usage here is a documented justified exception (same as `SqliteIdMapStore` — lock file requires atomic OS primitive not available via `IArtefactStore`). See `contracts/IPackageLockService.md` and `data-model.md` → "agent.lock".
...
- [ ] T013 Create `src/DevOpsMigrationPlatform.Infrastructure/Import/WorkItemFolderParser.cs` — static class with `static bool TryParse(string folderName, out long ticks, out int workItemId, out int revisionIndex)`. Returns `false` for comment folders (those whose names do not match the `{ticks}-{workItemId}-{revisionIndex}` pattern). Replace existing scattered `folderName.Split('-')` calls in `WorkItemImportOrchestrator.cs` with calls to `WorkItemFolderParser.TryParse`. See `research.md` → "Decision 6".

### Infrastructure — SkippedRevisionException

- [ ] T014 Create `src/DevOpsMigrationPlatform.Infrastructure/Import/SkippedRevisionException.cs` — module-private (internal) exception with `string Reason` property. Used by `RevisionFolderProcessor` to signal the orchestrator to write cursor as Completed and continue rather than failing the job.
...
- [ ] T015 Create `features/platform/package-lock/exclusive-package-lock.feature` — translate FR-017 and the "concurrent agent" edge case from `spec.md` into Gherkin scenarios:
  - Scenario: Second agent hard-bounced when live lock exists
  - Scenario: Stale lock (dead PID) is replaced and agent proceeds normally
...
- [ ] T017 Create `features/import/work-items/revisions/prevent-duplicate-work-items.feature` — translate User Story 1 acceptance scenarios from `spec.md` into Gherkin:
...
- [ ] T018 Update Stage A (CreatedOrUpdated) in `src/DevOpsMigrationPlatform.Infrastructure/Import/RevisionFolderProcessor.cs` — after existing idmap lookup, add deleted-target guard:
  1. If `idmap.GetTargetWorkItemIdAsync(sourceId, ct)` returns a value → call `target.WorkItemExistsAsync(targetId, ct)`.
  2. If target does NOT exist → call `await idMap.RecordSkippedRevisionAsync(folderPath, sourceId, targetId, "TargetWorkItemDeleted", ct)` → log structured error via `ILogger` → throw `SkippedRevisionException("TargetWorkItemDeleted")`.
  3. The orchestrator (caller) catches `SkippedRevisionException`, writes cursor as Completed, and continues with the next folder.
  See `data-model.md` → "Stage A — deleted target handling".

- [ ] T019 Update `src/DevOpsMigrationPlatform.Infrastructure/Import/WorkItemImportOrchestrator.cs` — catch `SkippedRevisionException` in the revision folder loop, write cursor as Completed for the folder, and continue to the next folder (no job abort).
...
- [ ] T020 Create `features/import/work-items/revisions/rebuild-idmap-from-target.feature` — translate User Story 2 acceptance scenarios from `spec.md` into Gherkin:
...
- [ ] T023 Create `features/import/work-items/revisions/rerun-delta-import.feature` — translate User Story 3 acceptance scenarios from `spec.md` into Gherkin:
...
- [ ] T025 Create `features/import/work-items/idmap-integrity-check.feature` — translate User Story 4 acceptance scenarios from `spec.md` into Gherkin:
...
- [ ] T026 Implement `CheckIntegrityAsync` as a private async method in `src/DevOpsMigrationPlatform.Infrastructure/Import/WorkItemImportOrchestrator.cs`:
  ```
...
- [ ] T027 Call `CheckIntegrityAsync` from `WorkItemImportOrchestrator.ImportAsync` — invoke it after `InitializeAsync` and `SeedAsync` complete, but before the streaming folder loop begins. Pass the same `CancellationToken`. See `data-model.md` → "Integrity Check" and `spec.md` FR-010.
...
- [ ] T028 Create `features/import/work-items/revisions/revision-level-progress-tracking.feature` — translate User Story 5 acceptance scenarios from `spec.md` into Gherkin:
...
- [ ] T029 Update revision folder processing in `src/DevOpsMigrationPlatform.Infrastructure/Import/WorkItemImportOrchestrator.cs` — before dispatching a folder to the processor, parse the folder name using `WorkItemFolderParser.TryParse` (T013) to extract `workItemId` and `revisionIndex`. Call `await idMapStore.GetLastRevisionIndexAsync(workItemId, ct)`. If `revisionIndex <= lastRevisionIndex` → skip the folder (do not write a cursor entry for this skip — cursor already passed it). Comment folders (`TryParse` returns false) are never subject to revision-index skip logic. See `research.md` → "Decision 6".
...
- [ ] T032 Update `.agents/context/checkpointing.md`:
  ...
  3. Add a note about the package lock: `At import job startup, an exclusive Checkpoints/agent.lock (JSON, contains jobId + PID) is acquired via IPackageLockService. A second agent targeting the same package is hard-bounced (PackageLockConflictException) if the owning process is still alive.`
...
- [ ] T036 Review `analysis/pending-actions.md` — remove or resolve any items that this spec addressed (idmap PostgreSQL→SQLite, revision tracking, package lock).

- [ ] T037 Run `dotnet clean && dotnet build --no-incremental` — **MUST pass with zero errors before this task is marked done**.

- [ ] T038 Run `dotnet test` — **ALL tests MUST pass before this task is marked done**.
...
Phase 1 (Foundational)  ← START HERE — no dependencies
    ↓ blocks all phases
Phase 2 (Package Lock)  ← depends on T004, T005, T011, T012
...
- T013 (WorkItemFolderParser) must be done before T029 and T030 (revision skip logic uses the parser)
