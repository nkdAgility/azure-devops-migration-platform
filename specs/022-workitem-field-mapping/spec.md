...
- Q: Dry-run mode (RT-M4)? → A: Included as part of prepare-time validation (FR-020). The `validate` method executes a sample dry-run against a configurable number of work items (default: 10), applying all transforms and verifying the output against target field definitions without writing. A full dry-run mode across all items is a future feature.
...
- **FR-020**: The FieldTransformTool MUST implement a `validate` method callable from the `prepare` command. This method MUST: (a) resolve all configured field references against source and target field definitions, (b) verify field type compatibility for each transform, (c) verify picklist values exist in the target for MapValueTransform targets, (d) execute a sample dry-run against a configurable number of work items (default: 10) — applying all transforms and verifying the output would be accepted by the target field definitions without actually writing, and (e) produce a structured validation report. Validation failures MUST block the migration from proceeding. *(RT-H2, RT-M4)*
