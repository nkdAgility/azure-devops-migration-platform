...
- What happens when the agent runs on a blob store with eventual consistency and reads `migration-config.json` before it is visible? → Agent must retry the read with back-off before falling back to defaults. A silent fallback on a new job is not acceptable.
...
- **FR-010**: The well-known package path for config (`migration-config.json`) MUST be defined as a constant in `PackagePaths` in `DevOpsMigrationPlatform.Abstractions` and referenced from all callers.
...
- **SC-005**: The job submission payload (`MigrationJob`) contains only `jobId`, `packageUri` (or `workingFolder`), `mode`, and `configVersion` — its size is constant regardless of the number or complexity of configured tool rules, endpoints, or policies.
...
- `MigrationJob` becomes a minimal pointer with four fields: `jobId`, `packageUri` (or `workingFolder` for local filesystem jobs), `mode`, and `configVersion`. Credentials and all other config travel exclusively via `migration-config.json`. This is a **breaking change** to the `MigrationJob` schema and requires a version increment and upgrader per guardrail Rule 9.
