...
- Export-side facet extraction — `CommentsWorkItemExtension` has `SupportsExport = true` and implements `ExportAsync`. `WorkItemRevisionExportContext` (new) carries per-revision export context. `WorkItemExportOrchestrator` routes to `CommentsWorkItemExtension` for comment export. Attachment binary download is unconditional inline code in the export orchestrator — attachments are core, not an extension.
- Corrective refactor — `LinksWorkItemExtension` and `AttachmentsWorkItemExtension` deleted entirely per Extension Seam Ethos (links and attachments are intrinsic to the work item entity's atomic save; they are not separate domain objects). `LinksExtensionOptions` and `AttachmentsExtensionOptions` deleted. `Links` and `Attachments` removed from `WorkItemsExtensionsOptions`. Link replay and attachment replay are unconditional core stages in `WorkItemResolutionProcessor`. `CommentsWorkItemExtension` remains the only valid `IModuleExtension` for WorkItems. Extension Seam Ethos written into `.agents/20-guardrails/core/capability-ethos-rules.md`.
