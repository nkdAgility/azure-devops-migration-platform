...
- [ ] T006a [P] Create `src/DevOpsMigrationPlatform.Abstractions/Options/IConfigSection.cs` — C# 11 static abstract interface member, guarded `#if NET7_0_OR_GREATER`: `public interface IConfigSection { static abstract string SectionName { get; } }`. Every options class that participates in schema registration MUST implement this interface. On `net481` compile slice the file compiles as empty (no type emitted). TFS agent never calls `AddSchemaEntry<T>()` — the guard is sufficient.
- [ ] T007 Create `src/DevOpsMigrationPlatform.Abstractions/Options/SchemaOptionsEntryExtensions.cs` — `AddSchemaEntry<T>(this IServiceCollection services, string? description = null) where T : class, IConfigSection` extension method (guarded `#if NET7_0_OR_GREATER`); reads `SectionPath` as `T.SectionName` — **no reflection required**; the generic constraint enforces `SectionName` existence at the `AddSchemaEntry<T>()` call site: if `T` does not implement `IConfigSection`, the call does not compile. Any options class missing a `SectionName` property is caught as a compile error, not a runtime exception.
...
- [ ] T033 [US5] Register `IConfigSchemaValidator` → `JsonSchemaConfigValidator` as singleton in `DevOpsMigrationPlatform.CLI.Migration` host startup; register `IOptions<JsonSchemaConfigValidatorOptions>` with `SchemaPath = Path.Combine(AppContext.BaseDirectory, "migration.schema.json")` — the options pattern allows tests to inject a different path without filesystem coupling
