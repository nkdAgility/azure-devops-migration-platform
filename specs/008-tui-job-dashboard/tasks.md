...
- [ ] T042 Run `dotnet test` from repository root — ALL tests must pass; no skipped tests introduced by this feature; Evidence: `dotnet test --no-build` failed in this reconciliation session after `dotnet clean` because multiple test assemblies were missing (e.g., `DevOpsMigrationPlatform.CLI.Migration.Tests.dll`, `DevOpsMigrationPlatform.Infrastructure.Agent.Tests.dll`). — Status: incomplete
...
- **T025/T030/T031/T033/T034/T035/T037/T043**: Only `tests/DevOpsMigrationPlatform.CLI.Migration.Tests/TUI/TuiAssemblyInitialize.cs` exists under the TUI test folder; the planned test files are absent.
- **T027**: `src/DevOpsMigrationPlatform.CLI.Migration/Views/TuiLogView.cs` currently implements `Trace/Logs/Metrics-Feed` mode rotation, not the specified Progress/Diagnostics two-mode contract.
- **T032**: `src/DevOpsMigrationPlatform.CLI.Migration/Commands/TuiCommand.cs` validates GUID format for `--job` but does not call `GET /jobs/{jobId}` to enforce existence/visibility before launching the TUI.
- **T042**: `dotnet test --no-build` from repo root failed after `dotnet clean` due to missing test assemblies, so no full-suite green evidence exists in this reconciliation.

## Evidence notes — superseded

- **T003/T009**: Separation-of-concerns abstraction moves are captured by `specs/021.2-separation-of-concerns/tasks.md` Phase 2/Phase 3.
- **T017/T018/T019/T020**: Command-surface consolidation to queue/prepare job submission is captured in `specs/025.1-fold-to-job/tasks.md` (notably T004).
- **T028**: Task-bootstrap/task-centric TUI layout supersedes the original three-panel interaction assumptions (`specs/028.1-task-bootstrap/tasks.md` T005).
