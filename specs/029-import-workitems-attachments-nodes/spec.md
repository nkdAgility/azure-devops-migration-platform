...
| Document | Status |
|---|---|
| `docs/modules.md` | Confirmed accurate — Module/Orchestrator/Service pattern, `IModule` contract, dependency graph rules |
...
| `.agents/context/package-format.md` | Confirmed accurate — `WorkItems/`, `Nodes/` layout, `idmap.db`, cursor schema |
| `.agents/context/checkpointing.md` | Confirmed accurate — cursor-based resume per revision folder |
| `.agents/guardrails/system-architecture.md` | Confirms: streaming is mandatory (rule 2), no in-memory sort (rule 3), cursors required (rule 4), attachments beside revision.json (rule 5), modules through `IArtefactStore`/`IStateStore` only (rule 7), Nodes before WorkItems (rule 8/dependency graph) |
...
- Read of docs: `docs/modules.md`, `docs/architecture.md`, `.agents/context/import-streaming.md`, `.agents/context/package-format.md`, `.agents/context/checkpointing.md`, `.agents/guardrails/system-architecture.md`, `analysis/proposed-features.md`.
...
| G-03 | **Field Mapping Tool** | `FieldMappingTool.ApplyFieldMappings()` — 10 map types (regex, value map, field-to-field, merge, clear, literal, tag, tree-to-tag, etc.) | No FR for arbitrary field transformations during import. Spec only covers identity and node path translation. | ⬜ **Separate spec** — too large for this story |
...
1. **G-03 (Field Mapping Tool)** — warrants a dedicated spec (e.g. `030-field-mapping-tool`). This spec notes it as an external dependency — users who need field-level transformations should await that spec.
2. **G-10 (Git commit link rewrite)** — warrants a separate spec covering link enrichment.
