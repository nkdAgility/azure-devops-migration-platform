Feature: Area and iteration path mapping
  ...
  Scenario: Regex mapping rule translates a matching area path
    Given a NodeStructure tool configured with area path mapping "^SourceProject\\(.*)" replaced by "TargetProject\$1"
  ...
  Scenario: Auto-swap translates source project name when no rule matches
    Given a NodeStructure tool with no mapping rules configured
  ...
  Scenario: Tool disabled bypasses all translation
    Given a NodeStructure tool that is disabled
  ...
  Scenario: Regex mapping uses capture groups for path restructuring
    Given a NodeStructure tool configured with area path mapping "^SourceProject\\Team A\\(.*)" replaced by "TargetProject\Merged\$1"
  ...
  Scenario: First matching rule wins when multiple rules are configured
    Given a NodeStructure tool configured with two area path mappings:
      | Match                              | Replacement              |
