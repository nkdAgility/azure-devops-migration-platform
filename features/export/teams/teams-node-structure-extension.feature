Feature: Teams NodeStructure Extension
  As a platform operator
  I want the Teams module to record all team area and iteration paths into the ReferencedPathTracker
  So that NodeStructureModule can pre-create those nodes on the target before teams are imported
  ...
  @import @teams @nodes
  Scenario: Import translates area paths via NodeTranslationTool
  ...
  @export @teams @nodes
  Scenario: NodeStructure extension disabled suppresses path recording
    Given the NodeStructure extension is disabled in TeamsModuleOptions
