...
| **O-2: Progress Events** | `IProgressSink.Emit(ProgressEvent)` | TUI live view, CLI `--follow`, Control Plane API | `Logs/progress.jsonl` in package; `GET /jobs/{id}/progress?follow=true` (SSE) |
| **O-3: Diagnostic Logs** | `ILogger` → `PackageLoggerProvider` | Operators troubleshooting failures | `Logs/agent.jsonl` in package; `GET /jobs/{id}/diagnostics?follow=true` (SSE) |
...
- `ILogger` writes are captured by `PackageLoggerProvider` and flushed to `Logs/agent.jsonl` before the job signals terminal state.
...
- [.agents/context/telemetry-architecture.md](../../.agents/context/telemetry-architecture.md) — detailed telemetry model
