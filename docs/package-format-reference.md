...
- Partial extraction (e.g., extracting only root `.migration/`, a project `.migration/`, or `.migration/runs/<runId>/` for audit inspection) is a supported tooling use case.
...
- **Determinism:** Given the same export input, two independent runs produce structurally identical packages (excluding `runId`, timestamps in `manifest.json`, and `.migration/runs/`).

### What Zip Does Not Affect

- Root package state (`.migration/`) is included in the zip and remains available after unpack.
- Project cursor state (`/{org}/{project}/.migration/`) is included in the zip and resumes correctly after unpack.
- Run-scoped audit folders under `.migration/runs/` are included as historical execution evidence.
