# Tests-First Session Workflow

**Audience:** Contributors (human and agent) running tests-first delivery sessions.
**Purpose:** The full session model — phases, handoffs, session logging, preconditions, and adoption paths — for the repository's tests-first delivery workflow.

The enforceable rules for this workflow live in
[`.agents/20-guardrails/workflow/test-first-workflow.md`](../.agents/20-guardrails/workflow/test-first-workflow.md).
This document explains how to run a compliant session.

One scenario = one session = one commit.
...
After ATDD intent is approved and before tests or production changes proceed, the session hardens the spec by running all of the following against the feature scope:
...
Specification hardening also records a **Capability Seam Decision** for each concern in scope:
...
Specification hardening also completes a non-skill architecture-perspectives gate for touched scope using `.agents/20-guardrails/core/architecture-perspectives-ethos.md`. This gate is mandatory even when architecture-review skills are not invoked.
...
Implementation execution also runs `.agents/commands/nkda-tddsn-autonomous.md` scoped to the subsystem under change. Its six output artefacts are mandatory implementation evidence for the session.
...
- [`failing-tests-workflow.md`](failing-tests-workflow.md) — procedure when tests fail
- [`testing-guide.md`](testing-guide.md) — testing model and conventions
- [`.agents/20-guardrails/workflow/test-first-workflow.md`](../.agents/20-guardrails/workflow/test-first-workflow.md) — the enforced rules
- [`.agents/20-guardrails/workflow/definition-of-done.md`](../.agents/20-guardrails/workflow/definition-of-done.md) — completion criteria
