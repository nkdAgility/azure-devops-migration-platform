...
15. **`Job` is the unit of work exchange.** CLI → `Job` (with `ConfigPayload`) → control plane → agent. All job kinds (`Inventory`, `Export`, `Prepare`, `Import`, `Validate`, `Migrate`, `Dependencies`) use the same `Job` class. The five pipeline phases — Inventory → Export → Prepare → Import → Validate — can each be run independently or chained via `Migrate`. TUI is a pure progress viewer — never submits jobs.
...
24. **Module/Tool/Analyser identifiers derived from class name.** `{Stem}Module`: `Name` = `"{Stem}"`, config = `MigrationPlatform:Modules:{Stem}`, cursor key = `{Stem}`, file = `{Stem}Module.cs`. `{Stem}Tool`: folder = `Tools/{Stem}/`, file = `{Stem}Tool.cs`, DI = `{Stem}ToolServiceCollectionExtensions.cs`, interface = `I{Stem}Tool`, options = `{Stem}Options`, config = `MigrationPlatform:Tools:{Stem}`. `{Stem}Analyser`: `Name` = `"{Stem}"`, config = `MigrationPlatform:Analysers:{Stem}`, DI extension = `Add{Stem}AnalyserServices` (or grouped within `AddAnalyserServices`), interface = `IAnalyser`, file = `{Stem}Analyser.cs`. Any deviation = instant reject.
