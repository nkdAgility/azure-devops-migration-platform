...
16. **The CLI must not contain migration logic.**
    The CLI parses arguments, builds a `MigrationJob`, starts the control plane in-process (for local and server execution) or connects to a remote endpoint, and renders progress. All migration logic — module execution, cursor writes, artefact reads/writes — lives in the Job Engine inside a Migration Agent. A CLI that calls a module directly, writes a cursor, or accesses `IArtefactStore` outside the Job Engine boundary is in violation.
...
20. **The control plane data store is PostgreSQL in all environments — no substitutions.**
    ...
    - **Local / Dedicated Server** (CLI-hosted, single machine): the CLI hosts the control plane in-process and starts PostgreSQL as a portable bundled child process. No Docker, no installer, no Aspire AppHost required. All services bind to localhost. Package storage is `file:///`.
