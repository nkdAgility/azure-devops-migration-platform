...
## Diagnostics Options (`queue`)

| Option | Default | Description |
|--------|---------|-------------|
| `--diagnostics` | `false` | Enables detailed CLI/control-plane request diagnostics and file exporters. If `Telemetry:DiagnosticsPath` is unset, the CLI sets it to `.otel-diagnostics` for the process and spawned standalone child processes. Raw bootstrap, telemetry, and progress payloads are written to the session `inbox/` folder in arrival order, with progress files named `progress-{module}-{stage}`. Only platform diagnostic categories are persisted there. |
