...
| `source` | Required for `Export` and `Migrate` | Source system connection — polymorphic by `type`. See Source/Target Endpoint Shapes below and [docs/source-types.md](source-types.md). |
| `target` | Required for `Prepare`, `Import`, and `Migrate` | Target system connection — polymorphic by `type`. `type` must be `AzureDevOpsServices` or `Simulated` (testing/development only). |
