...
29. **Guard clauses are compatibility-only and strategy-driven.** Runtime guards in module/orchestrator/service code are permitted only for runtime crash-prevention or unsupported runtime execution boundaries between `net481` and modern .NET targets (`net9.0`/`net10.0`). Defensive guards for nullable services, optional enablement toggles, or generic fail-fast checks are prohibited as local code guards and must be expressed through canonical validation surfaces. Current implementation gaps on `net481` are not a valid reason to add non-compatibility guards. Follow [runtime-compatibility-net10-net481.md](./runtime-compatibility-net10-net481.md) for hierarchy, capability modeling, and test obligations.

30. **Touched classes must be remediated to compliance before feature work.** If a class is changed and contains known non-compliance (guardrail violations, architecture drift, or missing behavioural tests for changed behaviour), remediation in the touched scope is mandatory and is the first task for that class. Deferring remediation is allowed only with explicit human approval of a bounded follow-up plan.
