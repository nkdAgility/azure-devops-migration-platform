...
///
/// <para>Each component is launched as a separate child process via <see cref="ChildProcessHost"/>.
/// This gives each component its own <c>System.Diagnostics.DiagnosticListener</c> instance,
/// eliminating the OpenTelemetry instrumentation bleed that occurs when multiple OTel pipelines
/// share a single process. The Application Insights Application Map shows the correct topology:
/// <c>CLI ↔ ControlPlane ↔ Agent ↔ dev.azure.com</c>.</para>
///
/// <para>If the published ControlPlane or MigrationAgent executables are not found,
/// an <see cref="InvalidOperationException"/> is thrown. Run <c>build.ps1 Install</c>
/// to publish the binaries before using standalone mode.</para>
///
...
public sealed class LocalStackHost : IAsyncDisposable
{
    ...
    /// Thrown when the ControlPlane or MigrationAgent executables are not found.
    /// Run <c>build.ps1 Install</c> to publish them.
    /// </exception>
    public async Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (controlPlaneExe is null || agentExe is null)
        {
            throw new InvalidOperationException(
                "ControlPlane or MigrationAgent executables not found. Run 'build.ps1 Install' to publish.");
        }
